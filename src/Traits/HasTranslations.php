...
trait HasTranslations
{
    ...
    */
    ...
    public function setTranslation(string $key, string|Locale $locale, mixed $value): static
    {
        ...
        $valueToStore = $this->isJsonTranslation($key) && is_array($value) ? json_encode(
            $value,
            JSON_THROW_ON_ERROR
        ) : $value;
    ...
    public function deleteTranslations(string|array|null $locales = null): void
    {
        ...
        $query = DB::table($this->getTranslationsTableName())->where(
    ...
    protected function resolveTranslatedValue(string $column, ?string $locale): ?string
    {
        ...
        $localesToCheck = array_unique(
            array_filter(
                [$locale, $this->getActiveLocale(), App::getLocale(), Config::get('translatable.fallback_locale')]
            ),
        );

        foreach ($localesToCheck as $currentLocale) {
            $translation = $this->loadedTranslations->where('column_name', $column)->where(
                'locale',
                $currentLocale
            )->first();
            if ($translation !== null) {
    ...
    {
        return !empty($this->translationTable) ? $this->translationTable : Str::singular(
                $this->getTable()
            ) . '_translations';
    }
